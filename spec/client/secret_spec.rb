describe Duse::Client::Secret do
  ...
  it 'should be able to marshall a secret correctly' do
    ...
    secret = Duse::Client::Secret.new title: 'test', required: 2, secret_text: secret_text
    Duse::Client::SecretMarshaller.new(secret, private_key, [], current_user, server_user).to_h
  end
  ...
  it 'should be able to handle more than self and server as users' do
    ...
    expect(secret.new?).to be true
    Duse::Client::SecretMarshaller.new(secret, private_key, [other_user], current_user, server_user).to_h
  end
end
